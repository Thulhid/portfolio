function Button({ children, disabled, variant, onClick }) {
  const base =
  ...
  const styles = {
    ...
    small:
      base +
      "rounded-xl px-4 transition-all duration-300 inset-shadow-sm bg-blue-100 text-gray-800 text-shadow-lg font-medium border-2 border-dashed border-blue-700",
  };

  return (
    <button disabled={disabled} onClick={onClick} className={styles[variant]}>
      {children}
    </button>
  );
}
